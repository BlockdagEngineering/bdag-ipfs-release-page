...
import {
  ...
  shellQuote,
} from "../releases/2.0.0-community-rescue-rc.62/assets/release-page.mjs";
...
test("a foreign published RC30 manifest cannot unlock the RC62 client", async () => {
...
test("the real RC62 manifest is the exact finalized publication and unlocks install commands", async () => {
  const manifestUrl = new URL("../releases/2.0.0-community-rescue-rc.62/release-manifest.json", import.meta.url);
  const manifest = JSON.parse(await readFile(manifestUrl, "utf8"));
...
test("the Pages privacy gate scans binary bytes and all private-key PEM headers", async () => {
  const root = await mkdtemp(join(tmpdir(), "rc62-pages-privacy-"));
  try {
...
test("RC62 requires software-only publication with both datasets pending", () => {
  ...
  const unsafeControllerPolicy = structuredClone(softwareOnly);
  unsafeControllerPolicy.runtime_change.custom_controller_policy = "permissive";
  assert.equal(publicationReady(unsafeControllerPolicy), false);

  const missingInventoryPolicy = structuredClone(softwareOnly);
  delete missingInventoryPolicy.runtime_change.privileged_profile_digest_inventory;
  assert.equal(publicationReady(missingInventoryPolicy), false);
...
test("generated software-only command is resumable, verified, and shell-valid", () => {
  ...
  assert.match(command, /BDAG_RELEASE_VERSION='2\.0\.0-community-rescue-rc\.62'/);
  assert.match(command, /BDAG_RELEASE_SEQUENCE='62'/);
...
test("the RC62 full archive RPC preset remains unavailable and fail closed", async () => {
  assert.deepEqual(
    resolveInstallSelection({
  ...
  const pageUrl = new URL("../releases/2.0.0-community-rescue-rc.62/index.html", import.meta.url);
...
test("neither pending RC62 dataset can produce an install command", () => {
  const manifest = syntheticPublishedManifest();
...
test("a foreign published RC44 release cannot unlock the RC62 client", async () => {
