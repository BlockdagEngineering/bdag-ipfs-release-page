...
pool_runtime_spec() {
  docker inspect pool | jq -ce '.[0] | {
    env:([.Config.Env[]?] | sort), entrypoint:.Config.Entrypoint, cmd:.Config.Cmd,
...
rollback() {
  ...
  set +e
  local status=rollback-failed first=-1 last=-1 deadline jobs metrics current_macs
  ...
  while ((SECONDS < deadline)); do
    ...
    if [[ $(docker inspect -f '{{.Image}} {{.State.Running}} {{.State.OOMKilled}}' pool 2>/dev/null) == "$old_pool_image_id true false" ]] \
      ...
      ((first<0)) && first=$last
      if ((expected_miners==0 || last>first)); then status=rolled-back; break; fi
    fi
    sleep 5
  done
  printf 'status=%s\nfinishedAt=%s\nexitStatus=%s\noldPoolImage=%s\nnewPoolImage=%s\narchiveManifestImage=%s\narchiveConfigImage=%s\npayoutAddress=%s\nasicConfigurationChanged=false\nstratumEndpointChanged=false\nrollbackShareDelta=%s\n' \
    "$status" "$(date -u +%FT%TZ)" "$rc" "$old_pool_image_id" "$runtime_image_id" \
    "$archive_manifest_id" "$archive_config_id" "$configured_payout" "$((last-first))" >"$evidence/result.receipt"
  write_sums || status=rollback-failed
  [[ $status == rolled-back ]] || rc=79
  exit "$rc"
}
phase=pre-cutover
failure_context() {
