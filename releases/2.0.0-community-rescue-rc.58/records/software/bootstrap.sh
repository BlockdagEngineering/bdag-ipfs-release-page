...
case "$ARCH_NAME" in
  x86_64|amd64)
    ...
    EXPECTED_SHA256='8f0b544a3682e79e283be296a3f148d9121139236c4ffbcc2152f51fded25eb0'
    ;;
  arm64|aarch64)
    PAYLOAD_TARGET='linux-arm64'
    EXPECTED_SHA256='2e40729208f90a24a9b3de9e853bfd6400d1e6c3682ee28f54d8c36c82aa57c4'
    ;;
  *)
...
if [ -f "$ZIP_PATH" ] && sha256sum -c "$CHECKSUM_PATH" >/dev/null 2>&1; then
  echo "Using previously completed $ASSET"
else
  rm -f "$ZIP_PATH"
  if [ -f "$ZIP_PATH.part" ]     && printf '%s  %s\n' "$EXPECTED_SHA256" "$ZIP_PATH.part"       | sha256sum -c - >/dev/null 2>&1; then
    echo "Using completed resumable download for $ASSET"
    mv "$ZIP_PATH.part" "$ZIP_PATH"
  else
    echo "Downloading or resuming $ASSET"
    curl --fail --location --show-error --progress-bar       --retry 5 --retry-delay 2 --retry-connrefused       --continue-at - -o "$ZIP_PATH.part" "$URL"
    mv "$ZIP_PATH.part" "$ZIP_PATH"
  fi
fi
