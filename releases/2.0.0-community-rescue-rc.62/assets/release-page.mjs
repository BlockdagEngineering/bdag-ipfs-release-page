...
// This module is release-specific. Every source, artifact, signature, and IPFS
// identity below is fixed to the qualified RC62 publication.
export const EXPECTED_RELEASE_IDENTITY = Object.freeze({
  ...
  releaseUrl: "https://github.com/BlockdagEngineering/stack/tree/2.0.0-community-rescue-rc.62",
  stackCommit: "60c3c6fd66edf24f79aff8ae8e0f91a3d01adb2e",
  ...
  sourceLockSha256: "09eef25d1a70af571ccce655162dfb579ff5836e62999900c2cda0fdabc7c729",
  recordsCid: "bafybeiabffabrb345zxeufuqh6kbj7qvyx3aprqe3mq2itn3uzjk72wsga",
  installer: {
    status: "published",
  ...
  targets: {
    "linux-amd64": {
      status: "published",
      filename: "pool-stack-docker-2.0.0-community-rescue-rc.62-linux-amd64.zip",
    ...
    "linux-arm64": {
      status: "published",
      filename: "pool-stack-docker-2.0.0-community-rescue-rc.62-linux-arm64.zip",
...
export function isCid(value) {
  ...
  return (
    value.length - 1 === Math.ceil((bytes.length * 8) / 5) &&
    bytes.length === 36 &&
    bytes[0] === 0x01 &&
    [0x55, 0x70].includes(bytes[1]) &&
    bytes[2] === 0x12 &&
    bytes[3] === 0x20
  );
}
...
function artifactMatches(actual, expected) {
  return Boolean(
    artifactReady(actual) &&
    expected &&
    actual.status === expected.status &&
    actual.filename === expected.filename &&
    actual.cid === expected.cid &&
...
function boundaryMatches(actual, expected, native = false) {
  if (!boundaryReady(actual, native) || !boundaryReady(expected, native)) {
    return false;
  }
  const numberKey = native ? "order" : "number";
  return (
    actual[numberKey] === expected[numberKey] &&
    actual.hash === expected.hash &&
    (native || actual.state_root === expected.state_root)
  );
}

function deliveryMatches(actual, expected) {
  if (!deliveryReady(actual) || !expected || actual.mode !== expected.mode) {
    return false;
  }
  if (actual.mode === "direct") {
    return (
      actual.cid === expected.cid &&
      actual.parts_manifest_path === expected.partsManifestPath &&
      Array.isArray(actual.parts) &&
      Array.isArray(expected.parts) &&
      actual.parts.length === 0 &&
      expected.parts.length === 0
    );
  }
  if (
    actual.cid !== null ||
  ...
  return actual.parts.every((part, index) => {
    const pinned = expected.parts[index];
    return Boolean(
      pinned &&
      part.filename === pinned.filename &&
      part.cid === pinned.cid &&
      part.sha256 === pinned.sha256 &&
      part.size_bytes === pinned.sizeBytes
    );
  });
}

function portableDatasetMatches(actual, expected) {
  return Boolean(
    datasetReady(actual) &&
    expected &&
    actual.status === expected.status &&
    actual.archive_node_equivalent === expected.archiveNodeEquivalent &&
    actual.version === expected.version &&
    actual.filename === expected.filename &&
    actual.sha256 === expected.sha256 &&
    actual.size_bytes === expected.sizeBytes &&
    actual.unpacked_size_bytes === expected.unpackedSizeBytes &&
    actual.canonical_manifest_path === expected.canonicalManifestPath &&
    actual.canonical_manifest_sha256 === expected.canonicalManifestSha256 &&
    actual.validation_spec_path === expected.validationSpecPath &&
    boundaryMatches(actual.native_boundary, expected.nativeBoundary, true) &&
    boundaryMatches(actual.evm_boundary, expected.evmBoundary) &&
    boundaryMatches(actual.fixed_checkpoint, expected.fixedCheckpoint) &&
    deliveryMatches(actual.delivery, expected.delivery)
  );
}

function datasetTrustMatches(actual, expected) {
  return Boolean(
    actual &&
    expected &&
    actual.dataset_key_path === expected.keyPath &&
    actual.dataset_key_id === expected.keyId &&
    actual.dataset_key_sha256 === expected.keySha256 &&
    actual.dataset_verifier_path === expected.verifierPath &&
    actual.dataset_verifier_sha256 === expected.verifierSha256
  );
}
...
export function publicationReady(manifest, expected = EXPECTED_RELEASE_IDENTITY) {
  ...
  const datasetTrustAbsent = (
    manifest.trust?.dataset_key_sha256 === null &&
    manifest.trust?.dataset_key_path === null &&
    manifest.trust?.dataset_key_id === null &&
    manifest.trust?.dataset_verifier_path === null &&
    manifest.trust?.dataset_verifier_sha256 === null
  );
  return Boolean(
    expected.publicationFinalized === true &&
    manifest.release.version === expected.version &&
    manifest.release.sequence === expected.sequence &&
    manifest.release.channel === expected.channel &&
    manifest.release.chain_id === expected.chainId &&
    manifest.runtime_change?.privileged_profile_digest_inventory === "exact-signed-inventory" &&
    manifest.runtime_change?.local_peer_controller === "stable-signed-wrapper" &&
    manifest.runtime_change?.interrupted_upgrade_retry === "authenticated-adopted-lineage" &&
    manifest.runtime_change?.custom_controller_policy === "fail-closed" &&
    manifest.reward_safety?.coinbase_accounting === "arbitrary-precision-wei" &&
    manifest.reward_safety?.automatic_payouts === "disabled-pending-staking-reconciliation" &&
    ...
    source.release_url === expected.releaseUrl &&
    source.tag === expected.version &&
    source.stack_commit === expected.stackCommit &&
    source.corechain_commit === expected.corechainCommit &&
    source.pool_commit === expected.poolCommit &&
    source.dashboard_commit === expected.dashboardCommit &&
    source.source_lock_sha256 === expected.sourceLockSha256 &&
    ...
    artifactMatches(manifest.installer, expected.installer) &&
    artifactMatches(software["linux-amd64"], expected.targets["linux-amd64"]) &&
    artifactMatches(software["linux-arm64"], expected.targets["linux-arm64"]) &&
    ...
    manifest.records_delivery?.cid === expected.recordsCid &&
    manifest.records_delivery?.path_prefix === "records/" &&
    manifest.trust?.release_key_sha256 === expected.releaseKeySha256 &&
    isSafeRelativePath(manifest.trust?.release_key_path) &&
    datasetTrustAbsent &&
    records.release_auth_manifest_path === "records/software/release-auth-manifest.json" &&
    records.release_auth_signature_path === "records/software/release-auth-manifest.json.sig" &&
    records.release_notes_path === "records/software/release-notes.md" &&
    qualification.signed_package_integrity_verified === true &&
    qualification.portable_dataset_verified === false &&
    qualification.full_archive_dataset_verified === false &&
...
export function buildInstallCommand(
  ...
  if (!publicationReady(manifest, expectedIdentity)) {
...
function renderArtifactCard(key, artifact, manifest, active) {
  ...
  const softwareArtifact = key.startsWith("linux-");
  const ready = softwareArtifact ? artifactReady(artifact) : datasetReady(artifact);
  const identityReady = softwareArtifact ? artifactIdentityReady(artifact) : datasetIdentityReady(artifact);
  const field = (name) => card.querySelector(`[data-field="${name}"]`);
  field("status").textContent = active && ready
    ? "Published and verified"
    : identityReady
      ? (key === "portable" ? "Qualified; CID pending" : "Signed; CID pending")
      : softwareArtifact ? "Not available" : "Not published";
  field("filename").textContent = artifact.filename || (softwareArtifact ? "Pending final metadata" : "None");
  field("size").textContent = artifact.size_bytes ? formatBytes(artifact.size_bytes) : (softwareArtifact ? "Pending" : "None");
  field("sha256").textContent = artifact.sha256 ? shortValue(artifact.sha256) : (softwareArtifact ? "Pending" : "None");
  if (field("unpacked-size")) {
    field("unpacked-size").textContent = artifact.unpacked_size_bytes ? formatBytes(artifact.unpacked_size_bytes) : "None";
  }
  ...
  if (!key.startsWith("linux-")) {
    if (artifact.delivery.mode === "direct") {
      ...
      href = immutableRecordUrl(manifest, artifact.delivery.parts_manifest_path);
      downloadLabel = "Open part manifest";
    }
  }
  field("cid").textContent = active && ready ? shortValue(cidLabel) : (softwareArtifact ? "CID pending" : "No delivery record");
  setLink(card.querySelector('[data-action="download"]'), active && ready ? href : null, active && ready ? downloadLabel : (softwareArtifact ? "Not yet published" : "Not published"));

  const manifestLink = card.querySelector('[data-action="manifest"]');
  if (manifestLink) {
    const recordHref = active ? immutableRecordUrl(manifest, artifact.canonical_manifest_path) : null;
    setLink(manifestLink, recordHref, recordHref ? "Signed manifest" : "No signed manifest");
  }
}

export function immutableRecordUrl(manifest, path) {
  if (!recordPathReady(manifest, path)) {
    return null;
  }
  const root = gatewayUrl(
    manifest.download_policy.ipfs_gateways[0],
    manifest.records_delivery.cid,
  );
  if (!root) {
    return null;
  }
  const relativePath = path.slice(manifest.records_delivery.path_prefix.length);
  try {
    return new URL(relativePath, `${root.replace(/\/$/, "")}/`).href;
  } catch {
    return null;
  }
}

function renderRecordLink(selector, path, manifest, active) {
  const link = document.querySelector(`[data-record="${selector}"]`);
  const code = link.parentElement.querySelector("code");
  const href = active ? immutableRecordUrl(manifest, path) : null;
  const datasetRecord = ["dataset-key", "dataset-verifier", "portable-manifest", "archive-manifest"].includes(selector);
  if (href) {
    link.href = href;
    ...
    link.setAttribute("aria-disabled", "true");
    code.textContent = datasetRecord && path === null ? "not published" : "pending";
  }
}

function renderManifest(manifest) {
  const active = publicationReady(manifest);
  const softwareReady = active && artifactReady(manifest.installer) && Object.values(manifest.software.targets).every(artifactReady);
  const softwareIdentity = active && artifactIdentityReady(manifest.installer) && Object.values(manifest.software.targets).every(artifactIdentityReady);
  const portableReady = active && datasetReady(manifest.datasets.portable);
  const archiveReady = active && datasetReady(manifest.datasets.full_archive);
  const datasetsAbsent = datasetPending(manifest.datasets.portable, false) && fullArchivePending(manifest.datasets.full_archive);
  ...
  document.getElementById("datasetStatus").textContent = active && datasetsAbsent ? "No RC62 dataset" : "Dataset state locked";
  document.getElementById("datasetStatusDetail").textContent = datasetsAbsent
    ? "Keep compatible chain data or synchronize normally"
    : "Unexpected dataset metadata keeps this release locked";
  setDot("releaseDot", active ? "ready" : "pending");
  setDot("softwareDot", softwareReady ? "ready" : "pending");
  setDot("datasetDot", active && datasetsAbsent ? "ready" : "pending");

  const notice = document.getElementById("draftNotice");
  notice.classList.toggle("published", active);
  notice.innerHTML = active
    ? "<strong>Installer correction published.</strong> Mining-profile payload staging now uses the complete signed digest inventory, and the local-peer controller follows the authenticated active release across interrupted upgrade retries."
  ...
  renderRecordLink("release-auth", manifest.software.records.release_auth_manifest_path, manifest, active);
  renderRecordLink("release-key", manifest.trust.release_key_path, manifest, active);
  renderRecordLink("release-notes", manifest.software.records.release_notes_path, manifest, active);
  renderRecordLink("dataset-key", manifest.trust.dataset_key_path, manifest, active);
  renderRecordLink("dataset-verifier", manifest.trust.dataset_verifier_path, manifest, active);
  renderRecordLink("portable-manifest", manifest.datasets.portable.canonical_manifest_path, manifest, active);
  renderRecordLink("archive-manifest", manifest.datasets.full_archive.canonical_manifest_path, manifest, active);

  const portableButton = document.querySelector('[data-dataset="portable"]');
  portableButton.disabled = !active || !portableReady;
  portableButton.setAttribute("aria-disabled", String(!active || !portableReady));
  const archiveButton = document.querySelector('[data-dataset="full_archive"]');
  archiveButton.disabled = !active || !archiveReady;
  archiveButton.setAttribute("aria-disabled", String(!active || !archiveReady));
...
function renderUnavailable() {
  document.getElementById("releaseBadge").textContent = "Unavailable";
  document.getElementById("releaseBadge").classList.remove("published");
  document.getElementById("releaseStatus").textContent = "Manifest unavailable";
  document.getElementById("releaseStatusDetail").textContent = "Do not download or install this release";
  document.getElementById("softwareStatus").textContent = "Software unavailable";
  document.getElementById("softwareStatusDetail").textContent = "No RC62 software identity was accepted";
  document.getElementById("datasetStatus").textContent = "Dataset unavailable";
  document.getElementById("datasetStatusDetail").textContent = "No RC62 dataset identity was accepted";
  for (const dot of ["releaseDot", "softwareDot", "datasetDot"]) {
    setDot(dot, "failed");
  }
  document.querySelectorAll('[data-action="download"], [data-action="manifest"]').forEach((link) => {
    setLink(link, null, "Not available");
  });
  document.querySelectorAll("[data-record]").forEach((link) => {
    link.removeAttribute("href");
    link.classList.add("disabled-link");
    link.setAttribute("aria-disabled", "true");
    link.parentElement.querySelector("code").textContent = "unavailable";
  });
  document.querySelectorAll('[data-dataset="portable"], [data-dataset="full_archive"], [data-preset="full-archive-rpc"]').forEach((button) => {
...
function initPage() {
  ...
  function renderCommand() {
    ...
    if (state.profile === "public-rpc") {
      ...
      element("#inputNotice").textContent = "This role does not require a payout wallet.";
      ...
      element("#inputNotice").textContent = "The public wallet format is valid. Confirm it independently before mining.";
    }
  }
  ...
  ["#dataDir", "#downloadDir", "#wallet"].forEach((selector) => element(selector).addEventListener("input", renderCommand));
