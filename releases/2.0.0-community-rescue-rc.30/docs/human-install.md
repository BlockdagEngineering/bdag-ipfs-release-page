# BlockDAG Community Rescue RC30

BlockDAG `2.0.0-community-rescue-rc.30`, release sequence `30`, is a qualified
community release candidate for mainnet chain ID `1404`. It includes bounded
startup retries for temporary RPC and peer-readiness conditions. A canonical checkpoint
or boundary mismatch, state-root mismatch, or network mismatch still fails immediately.

The RC30 software and portable v27 chain dataset are separate artifacts. You
may install RC30 while keeping compatible existing data, or restore portable
...
- `curl`, `unzip`, `sha256sum`, Python 3, `tar`, `zstd`, and OpenSSL.
...
2. Confirm it reports **Published release** and sequence `30`.
3. Select AMD64 or ARM64, mining or RPC, and either existing data or portable
   v27.
4. For mining, enter only the public payout address and ASIC MAC addresses.
5. Review and run the generated command.

The generated command verifies the signed release records, downloads every
artifact with IPv4 and HTTP/1.1, verifies each SHA-256, assembles the portable
dataset in order, verifies the complete archive, and then invokes the guarded
installer inside the selected signed package. The public path downloads
software directly from IPFS and requires no source-repository account or access
token.

## Reliable IPFS Downloads

Public IPFS gateways may time out while locating large blocks. Keep partial
files, resume them, and retry through another listed gateway. Always force IPv4
and HTTP/1.1:
...
The signed record must identify RC30, sequence `30`, the selected architecture,
...
Confirm the dashboard displays RC30, chain ID is `0x57c` (`1404`), canonical
