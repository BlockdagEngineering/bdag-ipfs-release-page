...
function renderManifest(manifest) {
  ...
  document.getElementById("datasetStatus").textContent = archiveReady ? "Portable and full archive published" : portableReady ? "Portable dataset published" : portableIdentity ? "Portable dataset recorded" : "No new dataset";
  document.getElementById("datasetStatusDetail").textContent = archiveReady
    ...
    : portableReady
      ...
      : portableIdentity
        ...
        : "RC52 is a software-only correction; keep compatible chain data in place";
  ...
  notice.innerHTML = active
    ? "<strong>Coinbase accounting restored.</strong> Future accepted blocks retain the complete node-issued reward through pool accounting. Historical wrapped credits remain unchanged and require separate reconciliation."
